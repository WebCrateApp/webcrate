...
import { domain, version } from '../utils/variables.js'
...
export async function renderMetaTags(req: express.Request, res: express.Response, next: express.NextFunction) {
	...
	if (req.originalUrl.startsWith('/crate/public')) {
		...
		// Replace image
		$(`meta[property='og:image']`).attr('content', `https://${ domain }/api/img/preview/${ crate.id }`)
...
export async function checkIfSetup(req: express.Request, res: express.Response, next: express.NextFunction) {
	...
	if (!setup) {
		...
		if (crates.length === 0) {
			...
			await createCrate('Read Later', 'Articles and blog posts I want to read later', 'book', false)
			await createCrate('Archive', 'Archive of old links', 'open_file_folder', false)
