...
export default {
	...
	async asyncData({ params, query, redirect, store, app: { $api } }) {
		...
		if (isExternal) {
			...
			const imageUrl = `https://${ crate.endpoint }/api/img/${ link.id }`
			const iconUrl = `https://${ crate.endpoint }/api/img/${ link.id }?type=icon`
		...
		const imageUrl = link.id === 'demo' ? link.meta.image : `/api/img/${ link.id }`
		const iconUrl = link.id === 'demo' ? link.meta.icon : `/api/img/${ link.id }?type=icon`
