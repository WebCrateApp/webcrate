...
export default {
	...
	computed: {
		...
		iconUrl() {
			if (this.endpoint) {
				return `https://${ this.endpoint }/api/img/${ this.link.id }?type=icon`
			} else {
				return `/api/img/${ this.link.id }?type=icon`
			}
		},
		imageUrl() {
			if (this.endpoint) {
				return `https://${ this.endpoint }/api/img/${ this.link.id }?type=image`
			} else {
				return `/api/img/${ this.link.id }?type=image`
			}
		},
		dragStartEvent() {
